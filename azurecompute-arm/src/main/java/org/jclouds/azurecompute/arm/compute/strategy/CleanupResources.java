...
public class CleanupResources {
   ...
   public boolean deleteResourceGroupIfEmpty(String group) {
      ...
      if (resourceGroupCanBeDeleted(group)) {
         ...
         URI delete = api.getResourceGroupApi().delete(group);
         if (delete != null) {
            deleted = resourceDeleted.apply(delete);
         }
      }
      return deleted;
   }
   ...
   private boolean resourceGroupCanBeDeleted(String group) {
      final String name = namingConvention.create().sharedNameForGroup(group);

      Predicate<org.jclouds.azurecompute.arm.domain.Resource> canBeDeletedWithRGPredicate = new Predicate<org.jclouds.azurecompute.arm.domain.Resource>() {
         @Override
         public boolean apply(org.jclouds.azurecompute.arm.domain.Resource input) {
            return !"Microsoft.Network/virtualNetworks".equals(input.type()) && name.equals(input.name()) && input.tags() != null &&
                  input.tags().containsKey("jclouds");
         }
      };

      Iterable<org.jclouds.azurecompute.arm.domain.Resource> resources =
              Iterables.filter(api.getResourceGroupApi().resources(group), canBeDeletedWithRGPredicate);
